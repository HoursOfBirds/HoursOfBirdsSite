...
let fedBirdsCount = parseInt(localStorage.getItem('birdFedCount')) || 0;
let sunCoins = parseInt(localStorage.getItem('birdSunCoins')) || 0;
...
// --- ЛОКАЛИЗАЦИЯ И ТЕКСТЫ (ФИКС СИНХРОНИЗАЦИИ) ---
// Читаем глобальный язык сайта из памяти (сохраненный на визитке)
...
function applyLocalization() {
    ...
    if (lang === 'auto') {
        // Если Авто - проверяем, вдруг есть глобальный язык, иначе берем из браузера
        lang = localStorage.getItem('siteLang') || (navigator.language.startsWith('ru') ? 'ru' : 'en');
    }
    
    // Подстраховка, если что-то пошло не так
...
function setLanguage(lang) {
    ...
    // Синхронизируем выбор внутри игры с глобальным сайтом
    if (lang === 'auto') {
        localStorage.removeItem('siteLang'); // Сбрасываем до системного
    } else {
        localStorage.setItem('siteLang', lang); // Сохраняем жестко
    }
...
// Запускаем локализацию при старте, учитывая глобальные настройки
document.addEventListener('DOMContentLoaded', () => {
    // Подсвечиваем нужную кнопку в меню языков при старте
    document.getElementById('lang-auto').classList.remove('active');
    document.getElementById('lang-ru').classList.remove('active');
    document.getElementById('lang-en').classList.remove('active');
...
function playSfx(type) {
    ...
    const maxFiles = SFX_CONFIG[type] || 1; 
    const randomNum = Math.floor(Math.random() * maxFiles) + 1; 
    let prefix = (type === 'meat') ? 'Meatfall' : 'Seedsfall';
    let filename = `${prefix}${randomNum}.ogg`; 
    
    let snd = new Audio('audio/' + filename);
    ...
    snd.play().catch(e => console.log('SFX block/missing:', filename));
}
            ...
            if (stillExistsIndex !== -1) {
                ...
                if (birdData.eats === 'enemy') {
                    ...
                    if (fedBirdsCount >= 100) {
                        fedBirdsCount -= 100;
                        sunCoins++;
                        localStorage.setItem('birdSunCoins', sunCoins);
                        updateSunUI();
                    }
                    localStorage.setItem('birdFedCount', fedBirdsCount);
                }
            } else {
